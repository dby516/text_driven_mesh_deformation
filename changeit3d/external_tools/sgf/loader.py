...
def initialize_and_load_sgf(sgf_code_dir=None, sgf_ae_ui_pkl=None):
    ...
    if sgf_code_dir is None:
       sgf_code_dir = os.path.expanduser('~/shapetalk/changeit3d/external_tools/sgf/ShapeGF/')
    
    sys.path.append(sgf_code_dir)
    
    if sgf_ae_ui_pkl is None:
        sgf_ae_ui_pkl = os.path.expanduser('~/shapetalk/changeit3d/external_tools/sgf/ShapeGF/SGF-latent-interface-pub.pkl')
    ae = None
    try:
