...
def run_all_metrics(transformed_shapes, gt_pcs, gt_classes, sentences, vocab, args, logger, 
    ...
    if args.pretrained_oracle_listener:
        oracle_listener = torch.load(args.pretrained_oracle_listener, map_location=device).eval().to(device)
    ...
    if args.pretrained_shape_classifier is not None:
        ...
        # shape_clf = torch.load(args.pretrained_shape_classifier, map_location=device).to(device)
        shape_clf = torch.load(args.pretrained_shape_classifier, map_location=device).to(device)
