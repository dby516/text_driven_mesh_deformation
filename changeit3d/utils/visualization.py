...
def plot_point_cloud_with_attention(pc, attention_scores, batch_index=-1, save_dir=None, visualization_pc_axis=(0, 1, 2)):
    """
    Plot a single 3D point cloud with attention scores as color intensity and save it to a file.
    Args:
        pc: Tensor/Array of shape [7540, 2048, 3], containing (x, y, z) coordinates of the point clouds.
        attention_scores: Tensor/Array of shape [7540, 2048], containing attention scores for each point cloud.
        batch_index: Index of the point cloud in the batch to visualize.
        save_dir: Directory where the plot image will be saved.
    """
    
    # Extract the specific point cloud and its attention scores
    if batch_index == -1:
        point_cloud = pc
        attention = attention_scores
    else:
        point_cloud = pc[batch_index]  # Shape: [2048, 3]
        attention = attention_scores[batch_index]  # Shape: [2048]
    if visualization_pc_axis != (0, 1, 2):
        point_cloud = swap_axes_of_pointcloud(point_cloud.copy(), visualization_pc_axis)
    

    # Normalize attention scores to [0, 1] for coloring
    attention = (attention - attention.min()) / (attention.max() - attention.min() + 1e-8)

    # Create a custom colormap with light blue as the lightest color
    colors = [(0.8, 0.9, 1), (0, 0, 1)]  # Define gradient from light blue to dark blue
    ...
    sc = ax.scatter(point_cloud[:, 0], point_cloud[:, 1], point_cloud[:, 2],
                    c=attention, cmap=custom_cmap, s=5)

    # Add color bar for attention intensity
    plt.colorbar(sc, label="Attention Score")
    plt.title(f"Point Cloud with Attention Scores (Batch {batch_index})")

    # Save the plot to a file
    save_path = f"{save_dir}/point_cloud_attention_{batch_index}.png"
    os.makedirs(f"{save_dir}", exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)  # Close the figure to avoid displaying it in interactive mode


def plot_3d_point_cloud(pc, show=False, show_axis=True, in_u_sphere=True, marker='.', s=8, alpha=.8, set_lim=1.1,
                        figsize=(5, 5), elev=10, azim=245, axis=None, title=None, visualization_pc_axis=(0,1,2), save_path=None, *args, **kwargs):
